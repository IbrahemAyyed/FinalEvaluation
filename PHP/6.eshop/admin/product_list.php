...
if (isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])) 
{
    ...
    if ($result->rowCount() == 1) 
    {
        ...
        if ($delete_result) 
        {
            ...
            // function file_exist() allows us to be sure that we got this picture registered on the server
            {
                unlink($picture_path);
    ...
    if (isset($_GET['m'])&&!empty($_GET['m'])) {
        switch ($_GET['m']) {
            case 'success':
                ...
                break;
                case 'fail':
                $msg_error .=  "<div class='alert alert-danger'>Error during the operation.If you till have this mistake after few tries, please call the dev' team.</div>";
                break;
                case 'update':
                $msg_success .=  "<div class='alert alert-success'>The update is successful</div>";
                break;
            default:
